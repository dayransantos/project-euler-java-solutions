...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    public Iterable<Key> keys(Key lo, Key hi) {
        ...
        keys(root, aggr, lo, hi);
        return aggr;
    }

    // add the keys between lo and hi in the subtree rooted at x
    private void keys(Node x, Collection<Key> aggr, Key lo, Key hi) {
        if (x == null) return;
        int cmplo = lo == null ? -1 : lo.compareTo(x.key);
        int cmphi = hi == null ?  1 : hi.compareTo(x.key);
        if (cmplo < 0) keys(x.left, aggr, lo, hi);
        if (cmplo <= 0 && cmphi >= 0) aggr.add(x.key);
        if (cmphi > 0) keys(x.right, aggr, lo, hi);
    }
